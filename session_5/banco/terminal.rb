  ...
  def operacoes(banco, cliente)
    ...
    while opcao != 0 do
      if opcao == 1
        ...
        deposito = gets.to_i
        depositos << deposito
        File.open("session_5/banco/database/#{cliente.cpf}-depositos.txt", 'a') { |f| f.write("#{cpf}|#{deposito}\n") }
        ...
        if saldo < saque
          ...
          saques << saque
          File.open("session_5/banco/database/#{cliente.cpf}-saques.txt", 'a') { |f| f.write("#{cpf}|#{saque}\n") }
